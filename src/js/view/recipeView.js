...
export const activeRecipe = id => {
    const oldActive = document.querySelector(`a.results__link.${elements.activeRecipeClass}`);
    const currentRecipe = document.querySelector(`a.results__link[href*="${id}"]`);
    if( oldActive ) oldActive.classList.remove(elements.activeRecipeClass);
    if( currentRecipe ) currentRecipe.classList.add(elements.activeRecipeClass);
}